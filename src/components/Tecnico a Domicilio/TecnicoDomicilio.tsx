  ...
  const ordenes = async () => {
    try {
      ...
      if (ordenes.length > 0 && clientes.length > 0) {
        // Mapear clientes por su id para facilitar la búsqueda
        const clientesMap = new Map(clientes.map((cliente: { id: any; }) => [cliente.id, cliente]));

        // Combinar información de cliente en cada orden
    ...
    <IonContent className="tecnico-domicilio-container">
      ...
      <div className="tecnico-domicilio-top-box">
        {ordenesList.map((orden: any) => (
          <div key={orden.numero_orden}>
            <h3><strong>Orden Activa</strong> #{orden.numero_orden}</h3>
            <h3>{orden.Cliente.nombre}</h3>
            <h3>
            ...
            <div>
              <IonButton style={{ borderRadius: "0" }} onClick={() => handleButtonClick('/verorden', orden)}>Ver orden</IonButton>
              <IonButton onClick={() => handleButtonClick('/chat', orden.cliente)}>Chat</IonButton>
            </div>
          </div>
        ))}
      </div>
