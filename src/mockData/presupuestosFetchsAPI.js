export const presupuestos = async () => {
  try {
    ...
    if (presupuestos[0] !== undefined) {
      console.log(
        `Se encontró un listado completo con ${presupuestos.length} presupuestos!!`
      );
...
const guardarPresupuesto = async (presupuesto) => {
  try {
    ...
    if (result) {
      ...
      console.log(
        "Se produjo un error, la presupuesto no pudo ser guardado..."
      );
      return false;
    }
  } catch (error) {
...
const modificarPresupuesto = async (id, presupuesto) => {
  try {
    const response = await fetch(
      `http://localhost:8000/presupuestos/modificar/${id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(presupuesto),
    ...
    if (result[0] === 1) {
      ...
      console.log(
        "Se produjo un error, el presupuesto no pudo ser modificado..."
      );
      return false;
    }
  } catch (error) {
...
const eliminarPresupuesto = async (id) => {
  try {
    const response = await fetch(
      `http://localhost:8000/presupuestos/eliminar/${id}`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      }
    ...
    if (result === 1) {
      console.log(
        "El presupuesto se eliminó correctamente de la base de datos!!"
      );
      return true;
    } else {
      console.log(
        "Se produjo un error, el presupuesto no pudo ser eliminado..."
      );
      return false;
    }
  } catch (error) {
